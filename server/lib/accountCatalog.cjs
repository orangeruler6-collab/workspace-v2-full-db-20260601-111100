...
const desktopLoginProfiles = [
  { id: 'login-profile-18', alias: 'profile-18', name: 'Lee小强', chromeProfileDirectory: 'Profile 18' },
  ...
  { id: 'login-profile-24', alias: 'profile-24', name: '畅玩白晓生', chromeProfileDirectory: 'Profile 24' },
  { id: 'login-profile-17', alias: 'profile-17', name: '超玩教授', chromeProfileDirectory: 'Profile 17' },
  ...
  { id: 'login-profile-7', alias: 'profile-7', name: '魁仔不想肝', chromeProfileDirectory: 'Profile 7' },
  ...
  { id: 'login-profile-15', alias: 'profile-15', name: '木游话说', chromeProfileDirectory: 'Profile 15' },
  ...
  { id: 'login-profile-33', alias: 'profile-33', name: '最翁damn', chromeProfileDirectory: 'Profile 33' },
...
const accountGroupFallbacks = [
  { groupName: '内容一组', groupId: 1, accounts: ['花无缺', '魁仔不想肝', '葵仔不想肝', '最翁damn', '最翁Damnnn', '薛定谔的机', '跑腿的包子', '李野王SG', '游电工厂', '硬件侠'] },
  { groupName: '内容二组', groupId: 2, accounts: ['痞仔伯爵', '暴走星号键', '雷鸭', '雷鸭Fist', '报告砖家', '网瘾少女一条'] },
  { groupName: '内容三组', groupId: 3, accounts: ['策划克星阿强', '中二探长', '团子好贵', '嘿小虎', '灵梦小师妹', '饭十七', '皮皮说游戏'] },
  ...
  { groupName: '内容六组', groupId: 6, accounts: ['不玩就分手', '游点慌', '游戏永动机', '畅玩白晓生', '畅玩百晓生', '夏洛', '游侠蹦蹦', '王路飞cp', '上官北', '上官北丶', '情风师兄', '鲁达大王'] }
];
...
const accounts = [
  {
    ...
    platforms: ['bilibili', 'kuaishou', 'xiaohongshu', 'wechatVideo', 'douyin'].map(platformId => ({
      id: platformId,
...
function enrichPlatform(platform) {
  ...
  const collectStatus = status && status.status ? status.status : (platform.login_status || 'unknown');
  return Object.assign({
    ...
    collect_status_reason: status && status.reason || '',
    collect_status_checked_at: status && status.lastCheckedAt || '',
    runnable: platformIsRunnable({ status: collectStatus })
  }, platform, {
...
function dashboardProfileMeta() {
  ...
  for (const account of catalogAccounts) {
    const value = {
      ...
      platformIds: runnablePlatformsForAccount(account).map(platform => platform.id).filter(Boolean)
    };
