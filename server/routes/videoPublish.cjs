...
function chromeProfileDirectory(accountId, profileAlias) {
  ...
  return process.env[envKey] || chromeProfileByOpenCli[profileAlias] || '';
}
...
async function ensureChromeProfileWindow(accountId, platformId, profileAlias, options) {
  ...
  const launched = await launchChromeProfile(profileDirectory, platformUrl(platformId) || 'about:blank');
  if (launched.ok) {
    ...
    while (Date.now() < waitUntil) {
      ...
      if (profiles.some(item => item.connected && (item.alias === profileAlias || item.context_id === profileAlias))) break;
    }
  }
...
module.exports = function createVideoPublishRoutes(deps) {
  ...
  function saveAccountBinding(input, auth, cb) {
    ...
    db.run(
      ...
      [
        ...
        cleanText(input.profile_alias || input.profileAlias, 120),
  ...
  function stageLabel(label, combined, data) {
    ...
    if (/wait product link input|douyin commerce addProduct/.test(text)) return '正在添加商品链接';
    if (/wait product modal|douyin commerce completeProduct/.test(text)) return '正在填写商品文案';
  ...
  async function runWithProfileLock(profileAlias, fn) {
    ...
    if (profileLocks.get(key)) throw new Error('This OpenCLI profile is already running another publish task. Please wait or cancel it first.');
  ...
  async function runDouyinJob(row, videoPath, profileAlias) {
    ...
    if (commerce && commerce.enabled) return runDouyinCommerceJob(row, videoPath, profileAlias, commerce);
    ...
    const args = [];
    if (profileAlias) args.push('--profile', profileAlias);
  ...
  function douyinCommerceScript(row, commerce, stage) {
    ...
    const productText = cleanText(commerce && commerce.productText, 80);
    return [
      '(function(){',
  ...
  async function runDouyinCommerceStage(row, commerce, prefix, session, combined, stage, timeoutMs) {
    ...
    if (false && stage === 'addProduct' && probed.data && !probed.data.ok) {
      const productUrl = cleanText(commerce && commerce.productUrl, 1000);
      const fallbackScript = `(function(){var productUrl=${jsUriLiteral(productUrl)};function textOf(el){return (el&&((el.innerText||el.textContent||el.value||el.placeholder)||'')||'').trim().replace(/\\s+/g,' ');}function visible(el){if(!el)return false;var r=el.getBoundingClientRect();var s=getComputedStyle(el);return s.display!=='none'&&s.visibility!=='hidden'&&r.width>0&&r.height>0;}function disabled(el){return !!(el.disabled||el.getAttribute('aria-disabled')==='true'||/disabled|disable|loading/.test(String(el.className||'')));}function setInput(el,value){if(!el)return false;el.focus&&el.focus();var proto=el.tagName==='TEXTAREA'?HTMLTextAreaElement.prototype:HTMLInputElement.prototype;var desc=Object.getOwnPropertyDescriptor(proto,'value');if(desc&&desc.set)desc.set.call(el,value);else el.value=value;el.dispatchEvent(new InputEvent('input',{bubbles:true,inputType:'insertText',data:value}));el.dispatchEvent(new Event('change',{bubbles:true}));return true;}function itemize(selector){return Array.from(document.querySelectorAll(selector)).map(function(el,i){var r=el.getBoundingClientRect();return {el:el,index:i,tag:el.tagName,text:textOf(el),cls:String(el.className||'').slice(0,180),placeholder:el.getAttribute&&el.getAttribute('placeholder')||'',type:el.getAttribute&&el.getAttribute('type')||'',disabled:disabled(el),visible:visible(el),x:Math.round(r.x),y:Math.round(r.y),w:Math.round(r.width),h:Math.round(r.height)};});}function clickItem(item){item.el.scrollIntoView({block:'center',inline:'center'});item.el.click();return {text:item.text,placeholder:item.placeholder,x:item.x,y:item.y,w:item.w,h:item.h};}var body=(document.body&&document.body.innerText||'').replace(/\\s+/g,' ');var fields=itemize('input,textarea').filter(function(item){return item.visible&&!item.disabled&&item.type!=='file'&&(/鍟嗗搧閾炬帴|绮樿创|閾炬帴|http|https/i.test(item.placeholder+' '+item.text)||item.w>260&&item.h>=24);}).sort(function(a,b){var as=/鍟嗗搧閾炬帴|绮樿创|閾炬帴|http|https/i.test(a.placeholder+' '+a.text)?0:1;var bs=/鍟嗗搧閾炬帴|绮樿创|閾炬帴|http|https/i.test(b.placeholder+' '+b.text)?0:1;return as-bs||b.w-a.w||b.y-a.y;});var input=fields[0];var inputOk=setInput(input&&input.el,productUrl);var buttons=itemize('button,[role=button],a,div,span').filter(function(item){return item.visible&&!item.disabled&&item.w>=30&&item.h>=18&&item.w<=260&&item.h<=90;});var add=buttons.filter(function(item){return /^(娣诲姞閾炬帴|娣诲姞鍟嗗搧|纭娣诲姞|纭|纭畾|娣诲姞)$/.test(item.text)||/娣诲姞閾炬帴|娣诲姞鍟嗗搧|纭娣诲姞/.test(item.text);}).sort(function(a,b){return b.y-a.y||b.x-a.x;})[0];if(add)clickItem(add);return {ok:inputOk&&!!add,input:inputOk,clicked:add&&add.text,placeholder:input&&input.placeholder,fields:fields.slice(0,8).map(function(item){return {text:item.text,placeholder:item.placeholder,x:item.x,y:item.y,w:item.w,h:item.h};}),buttons:buttons.filter(function(item){return /娣诲姞|纭|纭畾|鍟嗗搧|閾炬帴/.test(item.text);}).slice(0,30).map(function(item){return {text:item.text,x:item.x,y:item.y,w:item.w,h:item.h};}),tail:body.slice(-600)};})()`;
      const fallback = await evalBrowserJson(prefix, session, 'douyin commerce addProduct fallback', fallbackScript, combined, timeoutMs || 60000);
    ...
    if (stage === 'completeProduct' && probed.data && !probed.data.ok && /商品短标题|完成编辑/.test(String(probed.data.tail || ''))) {
      ...
      if (probed.data && !probed.data.ok) {
        ...
        const beforeFinishClickStderr = combined.stderr || '';
        await runBrowserStep(combined, 'douyin commerce click finish product edit', prefix.concat(['browser', session, 'click', '--text', '完成编辑']), 60000);
        combined.code = 0;
        combined.stderr = beforeFinishClickStderr;
  ...
  function douyinCommerceUploadReadyScript() {
    return String.raw`(function(){
      ...
      var rawUploading=/\u53d6\u6d88\u4e0a\u4f20|\u5f53\u524d\u901f\u5ea6|\u5269\u4f59\u65f6\u95f4|\u4e0a\u4f20\u4e2d|\u4e0a\u4f20\u8fc7\u7a0b\u4e2d\u8bf7\u4e0d\u8981|\u5df2\u4e0a\u4f20[：:]|\d{1,3}\s*%/.test(body);
      var stillUploadEntry=/\u70b9\u51fb\u4e0a\u4f20|\u6216\u76f4\u63a5\u5c06\u89c6\u9891\u6587\u4ef6\u62d6\u5165\u6b64\u533a\u57df/.test(body)&&!/\u91cd\u65b0\u4e0a\u4f20|\u9884\u89c8\u89c6\u9891|\u9884\u89c8\u5c01\u9762|\.mp4|\.mov/i.test(body);
      var uploadRunning=rawUploading||stillUploadEntry;
      var strongReady=Boolean(publishButton||/\u91cd\u65b0\u4e0a\u4f20|\u9884\u89c8\u89c6\u9891|\u9884\u89c8\u5c01\u9762|\.mp4|\.mov/i.test(body));
      var formReady=Boolean((title||editor||/\u4f5c\u54c1\u63cf\u8ff0|\u8bbe\u7f6e\u5c01\u9762|\u9009\u62e9\u5c01\u9762|\u5c01\u9762\u7f3a\u5931|\u6a2a\u5c01\u9762|\u7ad6\u5c01\u9762|\u53d1\u5e03\u8bbe\u7f6e|\u6dfb\u52a0\u6807\u7b7e|\u9884\u89c8\u89c6\u9891|\u9884\u89c8\u5c01\u9762|\u91cd\u65b0\u4e0a\u4f20|\u53d1\u6587\u52a9\u624b/.test(body))&&strongReady);
      return{ok:formReady&&!uploadRunning,formReady:formReady,uploadRunning:uploadRunning,titleFound:Boolean(title),editorFound:Boolean(editor),publishButtonFound:Boolean(publishButton),evidence:{rawUploading:rawUploading,stillUploadEntry:stillUploadEntry,strongReady:strongReady,hasCover:/\u8bbe\u7f6e\u5c01\u9762|\u9009\u62e9\u5c01\u9762|\u5c01\u9762\u7f3a\u5931|\u6a2a\u5c01\u9762|\u7ad6\u5c01\u9762/.test(body),hasPreview:/\u9884\u89c8\u89c6\u9891|\u9884\u89c8\u5c01\u9762/.test(body),hasReupload:/\u91cd\u65b0\u4e0a\u4f20/.test(body),hasDescription:/\u4f5c\u54c1\u63cf\u8ff0/.test(body)},tail:body.slice(-700)}
    })()`;
  }
...
var candidate=itemize('img,canvas,video').filter(function(item){return item.visible&&!item.disabled&&item.x>=dialog.x-40&&item.x<=dialog.x+dialog.w+40&&item.y>=dialog.y-40&&item.y<=dialog.y+dialog.h+40&&item.w>=50&&item.h>=50&&item.w<=720&&item.h<=720;}).sort(function(a,b){return a.y-b.y||a.x-b.x;})[0];if(candidate&&!window.__douyinCommerceCoverFramePicked){window.__douyinCommerceCoverFramePicked=true;return {ok:false,action:'select-cover-frame',coverMissing:coverMissing,target:clickItem(candidate)}}var confirm=pick(scoped.filter(function(item){return /^(纭畾|纭|瀹屾垚|淇濆瓨|涓嬩竴姝浣跨敤|搴旂敤)$/.test(item.text);}))||pick(buttons.filter(function(item){return /^(纭畾|纭|瀹屾垚|淇濆瓨|涓嬩竴姝浣跨敤|搴旂敤)$/.test(item.text);}));if(confirm){window.__douyinCommerceCoverFramePicked=false;return {ok:false,action:'confirm-cover-dialog',coverMissing:coverMissing,target:clickItem(confirm)}}return {ok:false,action:'cover-dialog-waiting',coverMissing:coverMissing,buttons:scoped.filter(function(i){return /灏侀潰|妯珫|纭畾|纭|瀹屾垚|璁剧疆|鏆備笉/.test(i.text+' '+i.ctx);}).slice(0,40).map(function(i){return {text:i.text,ctx:i.ctx,x:i.x,y:i.y,w:i.w,h:i.h};})};}if(!hasCoverSection)return {ok:true,skipped:'no cover section'};if(!coverMissing&&!/灏侀潰缂哄け|璁剧疆灏侀潰/.test(body))return {ok:true,coverMissing:false,reason:'cover ready'};var openCover=pick(buttons.filter(function(item){return /^(璁剧疆灏侀潰|閫夋嫨灏侀潰)$/.test(item.text)||/璁剧疆灏侀潰|閫夋嫨灏侀潰/.test(item.text)&&/灏侀潰缂哄け|妯皝闈绔栧皝闈璁剧疆灏侀潰/.test(item.ctx);}))||pick(buttons.filter(function(item){return /璁剧疆灏侀潰|閫夋嫨灏侀潰/.test(item.text+' '+item.ctx);}));if(openCover){window.__douyinCommerceCoverFramePicked=false;return {ok:false,action:'open-cover-picker',coverMissing:coverMissing,target:clickItem(openCover)}}return {ok:false,coverMissing:coverMissing,reason:'璁剧疆灏侀潰鎸夐挳鏈壘鍒?,buttons:buttons.filter(function(i){return /灏侀潰|妯珫|纭畾|纭|瀹屾垚|鍙戝竷/.test(i.text+' '+i.ctx);}).slice(0,40).map(function(i){return {text:i.text,ctx:i.ctx,x:i.x,y:i.y,w:i.w,h:i.h};}),tail:body.slice(0,1200)};})()`;
  ...
  async function ensureDouyinCommerceCover(prefix, session, combined) {
    ...
    combined.stdout += '\n$ wait douyin commerce cover card after force click local sleep 3s\n';
    await waitMs(3000);
    for (let i = 0; i < 18; i += 1) {
      ...
      const waitSeconds = /generate|generating|cover-dialog-wait|waiting|wait/.test(String(data.action || '')) ? '30' : '8';
  ...
  async function runDouyinCommerceAttempt(row, videoPath, profileAlias, commerce, session, prefix, attempt, combined) {
    ...
    const uploadResult = await runBrowserStep(combined, 'upload video', prefix.concat(['browser', session, 'upload', 'input[type=file]', videoPath]), videoUploadTimeoutMs(row, videoPath));
    ...
    if (combined.code !== 0) return combined;
    await step('wait product link input', prefix.concat(['browser', session, 'wait', 'time', '2']), 10000);
  ...
  function shouldSetDouyinCommerceCover(row, profileAlias) {
    ...
    return /dvabrcmr|楗崄涓億椋崄涓億fanshiqii|fan.?shi.?qi/i.test(accountText);
  }
  ...
  async function runDouyinCommerceJob(row, videoPath, profileAlias, commerce) {
    ...
    if (!cleanText(commerce && commerce.productText, 80)) throw new Error('Commerce mode is missing product copy');
    const session = ['commerce', row.platform_id, profileAlias || row.account_id || row.account_name || 'account', row.id || Date.now(), Date.now()]
      .join('-')
      .replace(/[^a-zA-Z0-9_-]/g, '-')
      .slice(0, 80);
    const prefix = [];
    if (profileAlias) prefix.push('--profile', profileAlias);
  ...
  async function executeJob(row) {
    ...
    return runWithProfileLock(profileAlias, async () => {
      ...
      try {
        ...
        const profileDir = launched && launched.profile_directory || chromeProfileDirectory(row.account_id, profileAlias);
        const profileClosed = await closeProfileDirectory(profileDir, 'job finished #' + row.id);
        if (result) {
          result.stdout = (result.stdout || '') + '\n$ close chrome profile after job\n' + stringify(profileClosed) + '\n';
        }
      }
    }).finally(() => {
  ...
  function queuedRetryCount(row) {
    ...
    const match = text.match(/鑷姩閲嶈瘯\((\d+)\/2\)/);
  ...
  async function rescheduleQueuedJob(row, error) {
    ...
    return new Promise(resolve => {
      db.run(
        ...
        ['scheduled', cleanText('鑷姩閲嶈瘯(' + (retryCount + 1) + '/2)绛夊緟涓細' + error, 1000), now() + delay, now(), row.id],
        function() {
          db.close();
          resolve(true);
        }
  ...
  return {
    ...
    '/api/video-publish/accounts/check-login': async function(body, cb) {
      ...
      const args = ['--profile', actualProfile].concat(commandByPlatform[platformId] || ['browser', 'publish-login-' + platformId, 'open', platformUrl(platformId) || 'about:blank']);
      ...
      if (result.code === 0 && platformId === 'wechatVideo') {
        probe = await probeWechatVideoPage(['--profile', actualProfile], 'publish-login-wechatVideo');
    ...
    '/api/video-publish/accounts/open-login': async function(body, cb) {
      ...
      const args = ['--profile', actualProfile, 'browser', session, 'open', url];
    ...
    '/api/video-publish/accounts/launch-profile': async function(body, cb) {
      ...
      const url = platformUrl(platformId) || 'about:blank';
      const profileDirectory = chromeProfileDirectory(accountId, profileAlias);
      if (!profileDirectory) { cb({ ok: false, error: '鏈厤缃?Chrome Profile 鐩綍', profile_alias: profileAlias }); return; }
      const launched = await launchChromeProfile(profileDirectory, url);
      cb(Object.assign({
    ...
    '/api/video-publish/jobs/cancel': async function(body, cb) {
      ...
      db.get(sql, args, async function(err, row) {
        ...
        const binding = await getBinding(row.account_id, row.platform_id);
        const profileAlias = await resolveProfileAlias(row.profile_alias || (binding && binding.profile_alias) || '');
        const profileDir = chromeProfileDirectory(row.account_id, profileAlias);
        const profileClosed = await closeProfileDirectory(profileDir, 'cancel job #' + id);
        await updateJobStatus(id, 'cancelled', '用户已取消发布任务', '');
        await updateJobProgress(id, '已取消', profileClosed.ok ? '已停止发布任务并关闭对应 Chrome Profile' : (killed ? '已请求停止正在执行的发布任务' : '任务已取消，未发现正在执行的子进程'));
        logRun({
          job_id: id,
          account_id: row.account_id,
          platform_id: row.platform_id,
          ...
          action: 'cancel',
          status: 'cancelled',
          stdout: stringify({ killed, profileClosed }),
          error: profileClosed.ok ? '用户已取消发布任务，已关闭对应 Profile' : (killed ? '用户已取消发布任务，已停止运行中的子进程' : '用户已取消发布任务')
        });
        cb({ ok: true, id, killed, profile_closed: profileClosed });
      });
    },
    ...
    '/api/video-publish/jobs/run-batch': function(body, cb) {
      ...
      db.all('SELECT * FROM video_publish_jobs WHERE id IN (' + placeholders + ')' + whereUser + ' ORDER BY id ASC', args, async function(err, rows) {
        ...
        if (err) { cb({ error: err.message }); return; }
        const results = [];
        const profileDirsToClose = new Set();
        try {
          for (const row of rows || []) {
            try {
              const binding = await getBinding(row.account_id, row.platform_id);
              const profileAlias = await resolveProfileAlias(row.profile_alias || (binding && binding.profile_alias) || '');
              const profileDir = chromeProfileDirectory(row.account_id, profileAlias);
              if (profileDir) profileDirsToClose.add(profileDir);
              const result = await executeJob(row);
          ...
          for (const profileDir of profileDirsToClose) {
            await closeProfileDirectory(profileDir, 'batch finished');
          }
        }
